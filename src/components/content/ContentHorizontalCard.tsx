...
const ContentHorizontalCard = ({ data, styleProp }: Props) => {
  ...
  return (
    <div
      ...
      className={`${styleProp} group relative w-[580px] h-[450px] cursor-pointer overflow-hidden`}
    >
      <img
      ...
      <div className="absolute bottom-12 left-0 flex items-center gap-10">
        <img
        ...
        <section className="flex flex-col gap-3">
          <VoteAverage data={data} style="w-12 h-12 text-xl" />
          <span className="flex flex-col gap-1">
            <p className="text-2xl font-bold text-left max-w-[320px] group-hover:text-blue-400 transition-all duration-75 ease-linear">
              {isMovie(data) ? data.title : data.name}
            </p>
            <p className="text-lg text-left text-white/60">
